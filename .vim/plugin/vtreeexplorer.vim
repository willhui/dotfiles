...
function! s:TreeExplorer(split, start) " <<<
	...
	if a:split || &modified
		...
		if winnum != -1
			" explorer window already opened, close it
			if winnr() == winnum
				" explorer window already active, close it (but don't close
				" it if it's the last window!)
				if winbufnr(2) != -1
					close
				endif
			else
				" switch to the explorer window and close it
				let curbufnr = bufnr('%')
				exe winnum . 'wincmd w'
				close
				" jump back to the original window if we're not already in it
	...
	if curfile =~ '/$' " dir
	  call s:ReadDir (ln, curfile)
		...
		elseif oldwin == winnr() || (&modified && s:BufInWindows(winbufnr(winnr())) < 2)
			" Commented out this line to fix a bug in VTreeExplore as described
			" in http://dailyvim.blogspot.com/2008/03/tree-explorer.html
			"
			" -Will
			"wincmd p
			exec ("new " . f)
